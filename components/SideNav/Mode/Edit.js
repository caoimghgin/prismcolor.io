...
export default function Main(props) {
  ...
  const onSave = (event) => {
    ...
    // Update URL parameters for the edited scale
    const currentPalette = searchParams.get('palette');
    if (currentPalette) {
      try {
        const paletteData = JSON.parse(decodeURIComponent(currentPalette));

        if (editing.id !== -1) {
          paletteData[editing.id] = {
            index: editing.id,
            semantic: value,
            keys: keyValues,
          };
          ...
          params.set('palette', encodeURIComponent(JSON.stringify(paletteData)));
          router.replace(`/create?${params}`);
        }
      } catch (e) {
        console.error('Failed to update palette data in URL', e);
      }
    }
